...
export default function TransitOSDashboard() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const res = await fetch(`${API_URL}/ledger_live`, { signal: AbortSignal.timeout(2000) });
      if (res.ok) {
  ...
  const handleReset = async () => {
    try { await fetch(`${API_URL}/reset_db`, { method: "POST" }); } catch {}
    setTickets([]);
    ticketIdRef.current = 0;
  };

  const totalRevenue = tickets.reduce((s, t) => s + t.total_fare, 0);
  const avgDist      = tickets.length ? tickets.reduce((s, t) => s + t.distance_km, 0) / tickets.length : 0;
          ...
          <div style={{ overflowX: "auto", maxHeight: 520, overflowY: "auto" }}>
            {tickets.length === 0 ? (
              <div style={{ padding: 60, textAlign: "center", color: "#4A7FA5", fontSize: 13 }}>
                No transactions yet — toggle LIVE SIM to populate
              </div>
            ) : (
